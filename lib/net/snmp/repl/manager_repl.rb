...
EOF
    ...
    def manage(options)
      options = { peername: options } if options.is_a?(String)
    ...
    #
    # - oids: A single oid, or an array of oids
    def walk(oids, _options = {})
      each_session do |session|
        session.walk(oids).each do |oid, value|
